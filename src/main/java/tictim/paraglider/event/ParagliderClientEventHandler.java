...
public final class ParagliderClientEventHandler {
        ...
        if (ModCfg.debugPlayerMovement()) {
            ...
            if (p != null) {
                ...
                if (h != null) {
                    ...
                    arr.add((h.isDepleted() ? ChatFormatting.RED : "") + "Stamina: " + h.getDoubleStamina() + " / " +
                            h.getDoubleMaxStamina());
                    arr.add(h.getStaminaVessels() + " Stamina Vessels, " + h.getHeartContainers() +
                            " Heart Containers");
                    arr.add(h.getRecoveryDelay() + " Recovery Delay");
                    arr.add("Paragliding: " + h.isParagliding());
                    arr.add("Stamina Wheel X: " + PERCENTAGE.format(ModCfg.staminaWheelX()) + ", Stamina Wheel Y: " +
                            PERCENTAGE.format(ModCfg.staminaWheelY()));
    ...
    public static void afterGameOverlayRender(RenderGameOverlayEvent.Post event) {
        ...
        int x = Mth.clamp((int) Math.round(ModCfg.staminaWheelX() * window.getGuiScaledWidth()),
                          1 + WHEEL_RADIUS,
                          window.getGuiScaledWidth() - 2 - WHEEL_RADIUS);
        int y = Mth.clamp((int) Math.round(ModCfg.staminaWheelY() * window.getGuiScaledHeight()),
                          1 + WHEEL_RADIUS,
                          window.getGuiScaledHeight() - 2 - WHEEL_RADIUS);
    ...
    public static void beforeGameOverlayLayerRender(RenderGameOverlayEvent.PreLayer event) {
		if (event.getOverlay() == ForgeIngameGui.CROSSHAIR_ELEMENT &&
			Minecraft.getInstance().screen instanceof StatueBargainScreen) {event.setCanceled(true);}
    }
