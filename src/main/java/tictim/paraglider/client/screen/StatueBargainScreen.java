...
public class StatueBargainScreen extends AbstractContainerScreen<StatueBargainContainer>
    ...
    public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (hasShiftDown())
            this.createdTime +=
                    newTimestamp - this.currentTickTimestamp; // For stopping multi item ingredient preview cycling
    ...
    protected void renderBg(PoseStack matrixStack, float partialTicks, int x, int y) {
        ...
        blit(
                ...
                4,
                17,
                SCROLL_BOX_THING_WIDTH,
                SCROLL_BOX_THING_HEIGHT,
                512,
                256);

        staminaWheelRenderer.renderStamina(
                matrixStack, getLeft() + SCROLL_BOX_THING_WIDTH + 5, getTop() - 5 - WHEEL_RADIUS, 0);
        ...
        if (dialog != null) {
            ...
            if (t >= DIALOG_FADEOUT_END) alpha = 0;
                ...
                alpha = Mth.clamp(
                        (int) ((DIALOG_FADEOUT_END - t) * 0xFF / (DIALOG_FADEOUT_END - DIALOG_FADEOUT_START)), 0, 0xFF);
        ...
        if (!bargains.isEmpty()) {
            ...
            for (BargainButton button : this.buttons) button.renderItems();
    ...
    protected void renderTooltip(PoseStack poseStack, int x, int y) {
        for (BargainButton button : this.buttons) button.renderToolTip(poseStack, x, y);
    }
    ...
    private void renderPreview(int bargainIndex) {
        ...
        for (int i = 0; i < demands.length; i++) {
            ...
            String s = (demand.getCount() >= demand.getQuantity()
                            ? demand.getCount()
                            : ChatFormatting.RED + "" + demand.getCount() + ChatFormatting.RESET)
                    + "/" + demand.getQuantity();
    ...
    private static float lerpAngle(double percentage, double start, double end) {
        return (float) Mth.lerp(
                percentage,
                start < end ? (end - start > 180 ? start + 360 : start) : (start - end > 180 ? start - 360 : start),
                end);
    }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        this.buttonIndexOffset = Mth.clamp(
                (int) ((mouseY - getTop() + 1 - 13.5) / (139 - 27) * offScreenBargains + .5), 0, offScreenBargains);
        return true;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.menu.getBargains().size() > 7
                && mouseX > left + 90
                && mouseX < left + 90 + 6
                && mouseY > top + 1
                && mouseY <= top + 1 + 139 + 1) this.isDragging = true;
    ...
    private final class BargainButton extends AbstractButton {
        ...
        public void renderItems() {
            ...
            for (int i = demands.size() - 1; i >= 0; i--) {
                ...
                int itemX = x
                        + determinePosition(i, demands.size(), BUTTON_INPUT_X_OFFSET_START, BUTTON_INPUT_X_OFFSET_END);
                ...
                if (demand.getQuantity() != 1)
                    itemRenderer.renderGuiItemDecorations(
                            font, stack, itemX, buttonElementTop, String.valueOf(demand.getQuantity()));
            }
            ...
            for (int i = offers.size() - 1; i >= 0; i--) {
                ...
                int itemX = x
                        + determinePosition(i, offers.size(), BUTTON_OUTPUT_X_OFFSET_START, BUTTON_OUTPUT_X_OFFSET_END);
                itemRenderer.renderAndDecorateFakeItem(stack, itemX, buttonElementTop);
                if (offer.getQuantity() != 1)
                    itemRenderer.renderGuiItemDecorations(
                            font, stack, itemX, buttonElementTop, String.valueOf(offer.getQuantity()));
            }
        }
