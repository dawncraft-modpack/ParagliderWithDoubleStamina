...
public class BargainScreenStaminaWheelRenderer extends StaminaWheelRenderer {
    private double internalStamina;
    private long lastUpdateTimestamp;
    private long timeSinceFull;
    private boolean gainedStamina;

    public BargainScreenStaminaWheelRenderer(double internalStamina) {
    ...
    protected void makeWheel(PlayerMovement h) {
        ...
        if (internalStamina > maxStamina) {
            ...
            for (WheelLevel t : WheelLevel.values()) addWheel(t, 0, t.getProportion(internalStamina), IDLE);
        } else if (gainedStamina && timeSinceFull < GLOW_FADE) {
            double stamina = ModCfg.maxStamina(h.getStaminaVessels() - 1);
            for (WheelLevel t : WheelLevel.values()) {
                addWheel(t, 0, t.getProportion(stamina), IDLE);
                addWheel(
                        t,
                        t.getProportion(stamina),
                        t.getProportion(maxStamina),
                        GLOW.blend(IDLE, (float) (timeSinceFull) / GLOW_FADE));
            }
        } else {
            for (WheelLevel t : WheelLevel.values()) addWheel(t, 0, t.getProportion(maxStamina), IDLE);
        }
    }
